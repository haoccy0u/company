...
func capacity_for(item_id: StringName, max_stack: int) -> int:
	if item_id == StringName() or max_stack <= 0:
		return 0
...
func add(item_id: StringName, amount: int, max_stack: int) -> int:
	if amount <= 0 or item_id == StringName() or max_stack <= 0:
		return 0
