...
func add_item(item_id: StringName, amount: int) -> int:
	if inventory == null:
		return 0
	var slot_capacity := _get_max_stack(item_id)
	var added := inventory.add(item_id, amount, slot_capacity)
	if added > 0:
		emit_signal("changed", self)
	return added
...
func place_one_from_cursor(index: int, cursor: InventorySlot) -> bool:
	...
	var slot_capacity := _get_max_stack(cursor.item_id)
	if slot_capacity <= 0:
		return false
	...
	if slot.item_id != cursor.item_id or slot.count >= slot_capacity:
		return false
