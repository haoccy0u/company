...
func build_from_roster(
	player_roster: Resource,
	...
	if player_roster == null or player_roster.get_script() != PlayerActorRosterRef:
		push_error("Squad.build_from_roster failed: invalid player_roster")
		return false
	if actor_catalog == null or actor_catalog.get_script() != ActorCatalogRef:
		push_error("Squad.build_from_roster failed: invalid actor_catalog")
		return false
	if selected_player_actor_ids.is_empty():
		push_error("Squad.build_from_roster failed: selected_player_actor_ids is empty")
		return false
	if member_scene == null:
		push_error("Squad.build_from_roster failed: member_scene is null")
		return false

	var members_root := _get_members_root()
	if members_root == null:
		push_error("Squad.build_from_roster failed: Members root missing")
		return false
	...
	for i in range(selected_player_actor_ids.size()):
		...
		if player_actor_id.is_empty():
			push_error("Squad.build_from_roster failed: selected player_actor_id is empty")
			_clear_members()
			return false
		if seen_player_actor_ids.has(player_actor_id):
			push_error("Squad.build_from_roster failed: duplicate player_actor_id=%s" % String(player_actor_id))
			_clear_members()
			return false
		seen_player_actor_ids[player_actor_id] = true

		var player_actor = player_roster.find_player_actor(player_actor_id)
		if player_actor == null:
			push_error("Squad.build_from_roster failed: player actor not found | player_actor_id=%s" % String(player_actor_id))
			_clear_members()
			return false

		var actor_def = actor_catalog.find_actor(player_actor.actor_id)
		if actor_def == null:
			push_error("Squad.build_from_roster failed: actor def not found | actor_id=%s player_actor_id=%s" % [
		...
		if member_node == null or member_node.get_script() != SquadMemberRef:
			push_error("Squad.build_from_roster failed: invalid member scene root script")
			_clear_members()
			return false
		...
		if member == null:
			push_error("Squad.build_from_roster failed: member cast failed")
			_clear_members()
			return false

		var ok := member.initialize_from_player(player_actor, actor_def, source_squad_id, i)
		if not ok:
			push_error("Squad.build_from_roster failed: member initialization failed")
			_clear_members()
			return false
